...
const activityStamp = (value = "") => {
  ...
  if (!/^\d{12}$/.test(raw)) return { date: raw || "—", time: "—" };
  return {
...
function activityTable(rows = []) {
  if (!rows.length) return `<p class="empty">No processed visitor activity detail is available yet.</p>`;
  const visible = rows.slice(0, 50);
  return `<div class="activity-table-wrap"><table class="activity-table"><thead><tr><th>Date</th><th>Time</th><th>City / Country</th><th>Device</th><th>OS</th><th>Browser</th><th>Landing page</th><th>Users</th><th>Sessions</th><th>Views</th><th>Avg. session</th></tr></thead><tbody>${visible.map((row) => { const stamp = activityStamp(row.dateHourMinute); return `<tr><td>${escapeHtml(stamp.date)}</td><td>${escapeHtml(stamp.time)}</td><td>${escapeHtml(row.city || "Unknown")}, ${escapeHtml(row.country || "Unknown")}</td><td>${escapeHtml(row.deviceCategory || "Unknown")}</td><td>${escapeHtml(row.operatingSystem || "Unknown")}</td><td>${escapeHtml(row.browser || "Unknown")}</td><td>${escapeHtml(row.landingPage || "/")}</td><td>${Number(row.activeUsers) || 0}</td><td>${Number(row.sessions) || 0}</td><td>${Number(row.views) || 0}</td><td>${escapeHtml(formatDuration(row.averageSessionDuration))}</td></tr>`; }).join("")}</tbody></table></div><div class="activity-count">Showing 1 to ${visible.length} of ${rows.length} rows</div>`;
}

function analyticsCsv(data) {
  const rows = data?.activityDetail || [];
  const q = (value) => `"${String(value ?? "").replace(/"/g, '""')}"`;
  const header = ["Date", "Time", "City", "Country", "Device", "OS", "Browser", "Landing page", "Users", "Sessions", "Views", "Avg. session seconds"];
  const lines = rows.map((row) => {
    const stamp = activityStamp(row.dateHourMinute);
    return [stamp.date, stamp.time, row.city, row.country, row.deviceCategory, row.operatingSystem, row.browser, row.landingPage, row.activeUsers, row.sessions, row.views, row.averageSessionDuration].map(q).join(",");
  });
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/analytics/activity.csv") {
      try {
        const csv = analyticsCsv(await getAnalyticsSnapshot(env));
        return new Response(csv, { headers: { ...securityHeaders, "Content-Type": "text/csv; charset=UTF-8", "Content-Disposition": "attachment; filename=kw-controls-active-user-details.csv" } });
      } catch (error) {
        return new Response(`Analytics export failed: ${error.message}`, { status: 500, headers: securityHeaders });
      }
    }
